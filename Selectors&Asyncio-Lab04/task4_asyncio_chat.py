...
async def handle_client(reader, writer): 
    ...
    try:
        while True:
            data = await reader.read(1024) [cite: 42]
            if not data: 
                break
            # broadcast f'{nickname}: {data.decode()}' [cite: 42]
        ...
        if writer in clients:
            ...
            del clients[writer]
            # broadcast 'nickname left' [cite: 42, 50]
            print(f' [-] {nickname} disconnected')
            await broadcast(f'{nickname} left\n')
        
        writer.close()
        await writer.wait_closed() [cite: 42, 37]

async def main():
    server = await asyncio.start_server(handle_client, HOST, PORT) [cite: 42]
